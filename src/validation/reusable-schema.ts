...
export const zNumber = (params: { name: string; min?: number; max?: number; mandatory?: boolean }): z.ZodNumber => {
...
export const zString = (params: { name: string; min?: number; max?: number; mandatory?: boolean }): ZodString => {
    const { name, max = 255, min = 1, mandatory = true } = params;
...
export const zPassword = (mandatory: boolean = true): TResultZPassword => {
    return zString({ name: 'Password', mandatory }).refine((val) => (mandatory ? regexValidation.password.test(val as string) : true), {
        message: messageError.password,
    }) as TResultZPassword;
};

export const zEmail = (mandatory = true) => {
    return zString({ name: 'Email', mandatory }).refine((val) => (mandatory ? regexValidation.email.test(val as string) : true), {
...
export const zEnum = <TEnum extends [string, ...string[]]>(params: { name: string; enum: TEnum; mandatory?: boolean }): z.ZodEnum<TEnum> | z.ZodOptional<z.ZodEnum<TEnum>> => {
...
export const zFileLocale = (params: { name: string; size?: number; listAcceptedTypeFile: TTypeFile[]; mandatory?: boolean }): z.ZodType<TFileValue> => {
