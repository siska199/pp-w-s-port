...
const HeroSection = () => {
    ...
    return (
        <ContainerSection ref={ref} gap="xl" className="mt-10 md:mt-0  min-h-screen !p-8" isLoading={isLoading[EPortfolioLoading.PERSONAL_INFORMATION]} loadingWording={'Loading Hero Section...'}>
            <Container gap="base" className="w-auto overflow-x-visible ">
                ...
                {isMaxMd && (
                    <motion.div {...opacityAnimation()} className="m-auto">
                        <Image className="mt-8 animate-bounce-custome  mx-auto rounded-full md:rounded-none w-[10rem] md:w-[25rem] aspect-square bg-transparent" src={data?.professional_image || ''} />
                    </motion.div>
                )}
