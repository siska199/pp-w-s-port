...
const FormSignIn = () => {
    ...
    const handleOnChange = useCallback(async (e: TEventOnChange) => {
        ...
        if (name === 'is_remember_me') {
            setItemSecureWebstorage(STORAGE_VARIABLE.IS_REMEMBER_ME, Boolean(value === 'true' ? true : false));
        }
    ...
    return (
        <Container variant={'vcc'} gap="base" className={` flex-nowrap  max-w-sm `}>
            ...
            <p className="mx-auto">
                ...
                <Button type="link" variant={'link-white'} className="underline" to={`${pathname}?type=sign-up`} onClick={(e) => e?.stopPropagation()}>
                    Sign Up
                </Button>{' '}
            </p>
        </Container>
