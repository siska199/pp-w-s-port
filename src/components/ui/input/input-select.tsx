    ...
    <ContainerInput<React.HTMLProps<HTMLInputElement>>
      ...
      childrenOverlay={
        <div
          ref={refContainerDropdown}
          className={clsx({
            'absolute   z-10 mt-0 origin-bottom-right rounded-md bg-white  ring-1 ring-black ring-opacity-5 focus:outline-none': true,
          ...
          {isOpen &&
            (isMultiple ? (
              ...
              <div onScroll={handleOnScroll} className='py-0 overflow-y-auto max-h-[10rem]'>
                ...
                {filteredOptions?.length == 0 && <div className='p-3 w-full !bg-gray-100  text-gray'>No Option</div>}
              </div>
            ))}
        </div>
      }
