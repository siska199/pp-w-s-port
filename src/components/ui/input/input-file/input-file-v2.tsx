...
const InputFileV2 = (props: TProps) => {
  ...
  const arrValue = useMemo(
    () => [
      ...Array.from(Array.isArray(attrs?.value) ? attrs.value : attrs.value ? [attrs.value] : [])
    ],
    [attrs.value]
  )
  ...
  const handleOnChange = async (params: File[]) => {
    ...
    const value = attrs?.multiple ? [...files, ...arrValue] : files[0]
    onChange({
      target: {
        name: attrs?.name,
        value
  ...
  const handleValidationInputFile = (file: File[] | File): boolean => {
    ...
    const totalSize: number = convertBytesToMegabytes(
      isMultiple ? file?.reduce((acc, data) => acc + data?.size, 0) : file?.size
    )
    ...
    const isAllFileTypesAllowed = isMultiple
      ? file?.every((sFile) => isValidTypeFile({ file: sFile, listAcceptedTypeFile }))
      : isValidTypeFile({ file, listAcceptedTypeFile })
  ...
  const handleRemoveItem = (index: number) => {
    ...
    const value = isMultiple
      ? Array.from(attrs?.value as File[])?.filter((_, i) => i !== index)
      : null
        ...
        <>
          <div className={'flex flex-col gap-4 w-full sm:w-[20rem]'}>
            <div
              ...
              className={cn({
                'flex flex-col items-center justify-center gap-2 rounded-md cursor-pointer-custome  text-center border-2 border-dashed  p-4 md:p-8 h-fit md:h-[10rem]':
                  true,
            ...
            >
              ...
              <p className='text-gray text-body-small text-center'>
