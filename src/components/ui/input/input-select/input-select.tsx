...
const InputSelect = (props: TPropsInputSelect) => {
    ...
    useEffect(() => {
        if (refInput?.current && isMultiple) {
            const width = searchQuery || !isEmptyValue(attrs.value) ? `${searchQuery?.length * 10 || 10}px` : '100%';
    ...
    const filteredOptions = useMemo(() => options?.filter((option) => String(option?.label)?.toLowerCase().includes(searchQuery?.toLowerCase())), [searchQuery, options]);
        ...
        <ContainerInput<React.HTMLProps<HTMLInputElement>>
            ...
            childrenOverlay={
                <div
                    ref={refContainerDropdown}
                    className={clsx({
                        'absolute  overflow-hidden z-10 mt-0 origin-bottom-right rounded-md bg-white  ring-1 ring-black ring-opacity-5 focus:outline-none': true,
                ...
                >
                    {isOpen &&
                        (isMultiple ? (
                            ...
                            <div onScroll={handleOnScroll} className="py-0 text-black overflow-y-auto max-h-[10rem]">
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={clsx({
                                                ...
                                                '!bg-gray-100': (isSearch && searchQuery && activeIndex == 1 && i === 0) || activeIndex - 1 == i,
                                            })}
                                        >
                                            {option?.label}
                                        </div>
                                ...
                                {filteredOptions?.length == 0 && <div className="p-3 w-full rounded-md !bg-gray-100  text-gray">No Option</div>}
                            </div>
                        ))}
                </div>
            }
