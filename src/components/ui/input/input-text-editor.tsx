...
const LazyEditor = React.lazy(() => import('react-draft-wysiwyg')?.then((module) => ({ default: module.Editor })));

interface TProps extends TBasePropsInput, Omit<React.ComponentProps<typeof LazyEditor>, 'onChange'> {
    value: string;
    name: string;
...
const InputTextEditor = (props: TProps) => {
    const { value, onChange, name, editorClassName, wrapperClassName, toolbar = basicToolbarConfig, ...attrs } = props;
