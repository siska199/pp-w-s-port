...
const InputDate = (props: TProps) => {
    ...
    const { name, value, iconPosition = 'start', placeholder, onChange, maxDate, minDate, ...attrs } = props;
    ...
    const date = useCallback((index: number = 0) => (Array.isArray(value) ? (value[index] ?? undefined) : (value ?? undefined)), [value]);
            ...
            {() => (
                <DatePicker
                    ...
                    renderCustomHeader={(propsCustomeHeader) => (
                        <HeaderDateCustome showTypeDate={showTypeDate} handleShowMonth={handleShowMonth} handleShowYear={handleShowYear} {...propsCustomeHeader} />
                    )}
...
const BtnIconPrevNext = (props: TPropsBtnIconPrevNext) => {
    const { type, decreaseMonth, decreaseYear, showTypeDate, prevMonthButtonDisabled, nextMonthButtonDisabled, increaseYear, increaseMonth } = props;
    const isNext = type === 'next';

    const handleOnClik = (e: React.MouseEvent<HTMLButtonElement | HTMLLinkElement, MouseEvent>) => {
        e.preventDefault();
        return ['year', 'month']?.includes(showTypeDate) ? (isNext ? increaseYear() : decreaseYear()) : isNext ? increaseMonth() : decreaseMonth();
    };
    return (
        <Button
...
const CustomeInput = forwardRef<HTMLInputElement, any>((props, ref) => {
    return <input {...props} className={`${props?.className} ${props?.disabled && '!cursor-default'}`} ref={ref} readOnly />;
});
