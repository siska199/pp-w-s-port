    ...
    >
      ...
      {value && (
        <div className={`${customeClass?.containerValue}`}>
          ...
          {type === TTypeDispalyInformation.FILE && typeof value === 'object' && (
            <DisplayFile file={value} />
          )}
          {type === TTypeDispalyInformation.HTML && typeof value === 'string' && (
            <DisplayInnerHMTL html={value} />
          )}
        </div>
      )}
    </div>
