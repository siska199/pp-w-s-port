...
import { IconArrowUp, IconDelete, IconEdit, IconEye, IconFastForward, IconFastRewind, IconSkipNext, IconSkipPrevious, IconSort } from '@assets/icons';

type WithId<T> = T & { id: string | number };
type WithOptionalChecked<T, TInclude extends boolean> = TInclude extends true ? T & { isChecked: boolean } : T & { isChecked?: never };
...
type TTableHeaderProps<TData, TIncludeChecked extends boolean = false> = Omit<TTableProps<TData, TIncludeChecked>, ''> & {
    isShowColumnAction?: boolean;
};

const TableHeader = <TData, TIncludeChecked extends boolean = false>(props: TTableHeaderProps<TData, TIncludeChecked>) => {
    const { withNo, columns, setting, isLoading, data, onChange, isShowColumnAction } = props;

    const handleSortColumn = useCallback(
        ({ key }: { key: keyof TData }) => {
            const sortDir = key !== setting?.sortBy ? 'desc' : setting?.sortDir === 'desc' ? 'asc' : 'desc';
...
type TTableBodyProps<TData, TIncludeChecked extends boolean = false> = Omit<TTableProps<TData, TIncludeChecked>, 'isLoading'> & {
    isShowColumnAction?: boolean;
};

const TableBody = <TData, TIncludeChecked extends boolean = false>(props: TTableBodyProps<TData, TIncludeChecked>) => {
            ...
            return Object.entries(actionBtn).map(([key, onClick], i) => {
                ...
                return (
                    <Tooltip key={i} text={tooltipInformation} variant="bottom">
                        <Button key={key} className="" variant={variant} shape="circle" onClick={() => onClick(dataRow)}>
                            {Icon}
                        </Button>
                    </Tooltip>
                );
            ...
            {data?.map((dataRow, i) => {
                return (
                    <tr key={i} className="border-b border-warning-100">
                        {withNo && <td className={`p-th-td `}>{(setting?.currentPage - 1) * setting?.itemsPerPage + i + 1}</td>}

                        {columns?.map((column, j) => (
                            <td key={j}>
                                <div className={`flex p-th-td  ${column?.className}`}>{column?.customeComponent ? column?.customeComponent(dataRow) : (dataRow?.[column.key] as string)}</div>
                            </td>
                        ))}

                        {isShowColumnAction && (
                            <td>
                                <div className="min-w-[5rem] overflow-visible flex justify-center items-center pr-2">{renderActionButtons(dataRow)}</div>
                            </td>
                        )}
                    </tr>
                );
        ...
        <div className="flex w-full justify-end px-4 py-2">
            <div className=" flex border rounded-md overflow-hidden">
                {listButton?.left?.map((btn, i) => (
                    <Button title={btn.title} key={i} onClick={btn.onClick} variant="no-style" className="rounded-none !border-r h-full" disabled={setting.currentPage == 1}>
                        {btn.icon}
                    </Button>
                ))}
                ...
                {listButton?.right?.map((btn, i) => (
                    <Button title={btn.title} key={i} onClick={btn.onClick} variant="no-style" className="rounded-none !border-r h-full" disabled={setting.currentPage == setting.totalPage}>
                        {btn.icon}
                    </Button>
                ))}
            </div>
