    ...
    <div className=" min-h-[calc(100%-5rem)] text-white px-8 my-8 md:my-0 space-y-4 lg:space-y-2  flex flex-col items-center justify-center">
      ...
      <div className="flex flex-col-reverse md:flex-row gap-8 ">
        <div className="flex-grow min-w-[20rem] gap-2">
          {listInformation?.map?.((info, i) => (
            <CardInfo key={i} {...info} />
          ))}
        </div>
        <p className=" indent-16 md:text-body-large h-auto my-auto text-white p-4 rounded-md bg-gray-50/30  ">
          ...
          Axios. Expertise in component architecture, custom hooks, implementing
          Figma designs from UIUX, creating responsive layouts, and developing
          design systems using various CSS frameworks such as Tailwind,
          Bootstrap, and Styled Components. Proven ability to work
          collaboratively in a team, successfully delivering projects on tight
          deadlines. Eager to expand my skills in backend development to further
          enhance my capabilities in creating full-stack applications.
        </p>
      </div>
    </div>
  );
...
const CardInfo = (props: TPropsCardInfo) => {
  ...
  return (
    <div className="flex gap-1 items-center  p-4 rounded-lg effect-split-bg  ">
      <h1 className="text-heading-05 md:text-heading-01 font-bold w-[5rem] md:w-auto ">
        {title}
      </h1>
