...
export const mappingErrorsToForm = <TSchema, TForm extends TObject>(params: TMappingErrorsToForm<TSchema, TForm>) => {
...
export const fetchOptions = async <T>(fetchFunction: (params?: any) => Promise<T[]>, params?: any) => {
...
export const formatDate = (params: TFormatDate): string => {
    let { date } = params;
    const { format = TTypeDateFormat['DD/MM/YYYY'], timeZone = 'UTC', formatLanguage = 'en-GB' } = params;
...
export const filterKeysObject = <TObject extends object>(params: TParamsFilterKeysObject<TObject>) => {
